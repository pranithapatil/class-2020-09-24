...
```{r}
sep_now <- inner_join(sep_new, sep_old, by=c("id","name"), suffix = c("_new", "_old")) %>%
...
```{r}
rbind(sep_now_dec, sep_now_inc) %>%
  ggplot(aes(fct_reorder(name,difference), difference, fill = department_new)) +
  geom_col() +
